...
7. 没有 key 时继续使用 Fake/replay 完成本地测试；只配置模型或只配置 key 的部分 live 配置必须 fail-closed。2026-08-23 已完成一次真实 `deepseek-v4-flash` 只读 smoke（session `completed`，8 轮、12 次只读工具调用、148,994 tokens、约 4 分 28 秒），证明一次端到端 Chat Completions 请求可用；成本、长任务、写操作、故障恢复和其他模型仍未验证。

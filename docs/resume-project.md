...
- 建立 deterministic Fake/replay 与跨平台回归体系；上一已绑定 clean 候选（`833e3aaf...`）在 Windows 11 为 `472 passed, 5 skipped`，并通过 Ruff、mypy、lock、构建、packaged CLI smoke、clean preflight、固定 Docker Demo 和匹配 target SHA 的 GitHub Actions；后续执行策略加固已纳入 `main`，本机结果为 `511 passed, 5 skipped`，最终发布仍须绑定选定 clean `HEAD` 的 manifest 和 CI。独立 WSL2 矩阵尚未刷新，不能沿用历史数字。
...
- Developed deterministic replay/fake providers and cross-platform regression coverage; the previously bound clean candidate (`833e3aaf...`) passed 472 Windows tests with 5 platform skips, plus lint, typing, locked builds, packaged CLI smoke, clean preflight, the fixed Docker demo, and matching Windows/Ubuntu GitHub Actions. Subsequent process-policy hardening is now on `main` and reruns at 511 passed with 5 skips locally; final release evidence still requires a manifest and CI run bound to the selected clean `HEAD`. The standalone WSL matrix remains a separate unverified boundary.
